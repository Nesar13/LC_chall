class Solution {
    ...
    public List<List<Integer>> pacificAtlantic(int[][] matrix) {
        ...
        // we know that the first column is reachable to pacific so we add
        // to queue and the last queue to atlantic queue
        for (int i = 0; i < numRows; i++) {
            pacificQueue.offer(new int[]{i, 0});
            atlanticQueue.offer(new int[]{i, numCols - 1});
        }
        // we add first row to pacific and last row to atlantic 
